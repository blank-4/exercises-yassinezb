...
request.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
        var toto = this.responseText;
        var objectAPI = JSON.parse(toto);;
    for (i = 0; i < objectAPI.records.length; i++) {
        var div = document.createElement("div");
        document.body.appendChild(div);
        var a = objectAPI.records[i].fields.ap_cp;
        var b = objectAPI.records[i].fields.ap_num; 
        var c = objectAPI.records[i].fields.ap_voie;
        var d = objectAPI.records[i].fields.designation_longue;
        div.innerHTML = a + " " + b + " " + " "+ c +" " + d;
    }
        /* p.innerHTML = toto;  */
